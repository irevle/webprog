...
<body>
    ...
    <?php if (isset($_COOKIE["roti"])) {
        ...
        $urut = isset($_COOKIE["settingUrut"])
            ? $_COOKIE["settingUrut"]
            : "tanggal";
        $arah = isset($_COOKIE["settingArah"])
            ? $_COOKIE["settingArah"]
            : "ascend";
        echo "<ul>";
        ...
        if ($urut == "tanggal") {
            if ($arah == "ascend") {
                ...
                //urutin sesuai descending tanggal
                for ($i = 0; $i < count($key); $i++) {
                    for ($j = 0; $j < count($key) - 1; $j++) {
                        if ($key[$j] < $key[$j + 1]) {
                            $simpanKey = $key[$j];
            ...
            if ($arah == "ascend") {
                ...
                // urutin sesuai descending nominal
                for ($i = 0; $i < count($value); $i++) {
                    for ($j = 0; $j < count($value) - 1; $j++) {
                        if ($value[$j] < $value[$j + 1]) {
                            $simpanValue = $value[$j];
